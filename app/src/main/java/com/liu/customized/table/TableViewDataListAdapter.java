...
public class TableViewDataListAdapter extends ArrayAdapter<TableViewRowBean> {
...
//    private TableViewDataListAdapter.GridViewListAdapterListener mListener;
    ...
    public void setDataRows(List<TableViewRowBean> rows)
    {
        this.mRowDatas = rows;
    }
    public TableViewDataListAdapter(Activity context,
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        ...
        TableViewRowBean rowItem = mRowDatas.get(position);
...
//            final RecyclerView rowRecyclerView = (RecyclerView) convertView.findViewById(R.id.grid_view_row_rv);
            ...
            if (mWrapRowFlag)
            {
                ...
                viewHolder.rowNumber.setWidth((int)(DisplayUtils.getDisplayFontPx(CustomizedTableView.CELL_FONT_SIZE)
                        * rowNumberTextLength)
                );
...
//                }
            ...
            TableViewDataRowAdapter gridViewAdapter = (TableViewDataRowAdapter)viewHolder.rowData.getAdapter();
            if (gridViewAdapter != null)
            {
                final TableViewDataRowAdapter adapter = gridViewAdapter;
            ...
            else
            {
                ...
                {
                    gridViewAdapter = new TableViewDataRowAdapter(mContext, mTotalColumnSpan, mFixColumnCount, mHeaders.get(0).getColumnCells(), rowItem.getColumnCells(), mWrapRowFlag, mAllRowExpandFlag, this.mShortTextFlag);
                }
                else
                {
                    gridViewAdapter = new TableViewDataRowAdapter(mContext, mTotalColumnSpan, mFixColumnCount, mHeaders.get(0).getColumnCells(), rowItem.getColumnCells(), mWrapRowFlag, mInitColumnCount, this.mShortTextFlag);
                }
                gridViewAdapter.setOnGridViewRowCellActionListener(new TableViewDataRowAdapter.OnGridViewRowCellActionListener() {
                    @Override
                    public void onFirstRowDoubleClicked(Object sender, View v) {
                        viewHolder.rowData.setAdapter((TableViewDataRowAdapter)sender);
                    }
                });
